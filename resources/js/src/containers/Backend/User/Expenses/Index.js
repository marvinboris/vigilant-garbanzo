...
import { faEye, faEdit, faTrash, faWallet } from '@fortawesome/free-solid-svg-icons';
...
class Index extends Component {
    ...
    render() {
        let {
            content: {
                cms: {
                    pages: { components: { list: { action } }, backend: { pages: { expenses: { title, add, index, form } } } }
                }
            },
        ...
        const data = expenses.map(claim => {
            return updateObject(claim, {
                action: <div className="text-center">
                    <Link to={`/user/expenses/${claim.id}`} className="mr-2">
                        <FontAwesomeIcon icon={faEye} className="text-green" fixedWidth />
                    </Link>
                    {JSON.parse(feature.permissions).includes('u') && <Link to={`/user/expenses/${claim.id}/edit`} className="mx-1">
        ...
        const content = (
            <>
                <Row>
                    <List array={data} loading={loading} data={JSON.stringify(expenses)} get={this.props.get} total={total} bordered add={add} link="/user/expenses/add" icon={faWallet} title={index} className="shadow-sm"
                        fields={[
        ...
        return (
            <>
                <TitleWrapper>
                    <Breadcrumb main={index} icon={faWallet} />
                    <SpecialTitle user icon={faWallet}>{title}</SpecialTitle>
